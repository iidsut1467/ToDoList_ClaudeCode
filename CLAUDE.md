...
### 拡張の方針（設計メモ）
- データの1件は現在 `{id, text, done}`。今後はここに項目を足していく:
  `{id, text, done, dueAt, priority, difficulty, estimateMin, remindAt, ...}`。
  **古いデータでも壊れないよう、新項目は「無ければ既定値」で扱う**（後方互換）。
- バックエンドは `api.php` と `server.rb` の **2つを常に同じ仕様に揃える**（片方だけ直さない）。
- フロントは `app.js` の `render()`（表示）と入力フォーム、`api.php` 側の保存項目をセットで増やす。
- カレンダー連携は外部API（Google Calendar等）の認証が絡み一番重いので最後に着手。

### 進める順番（提案）
- フェーズ1（土台）: データ項目を拡張 — 期限 / 優先度 / 難度 / 所要時間 の入力・表示。
- フェーズ2: 期限からの自動優先順位づけと並べ替え、優先度の見える化。
